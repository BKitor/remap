...
/* This file is directly copied from ompi/mca/coll/base/coll_base_allreduce.c
 * but with some additional modification for CUDA optimised collective
 * communication
 */

#include "ompi_config.h"
#include "coll_remap.h"
#include "ompi/mca/coll/remap/kernel/kernel.h"
...
/*
 *   ompi_coll_cuda_allreduce_opt_recursivedoubling
 *
                                                         ...
                                                         coll_remap_allreduce_cuda_coll_config_t *cuda_config)
{
    ...
    mca_coll_remap_module_t *s = (mca_coll_remap_module_t *)module;

    size = ompi_comm_size(comm);
    rank = ompi_comm_rank(comm);

    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:remap:allreduce_recursivedoubling_cu_reudce rank %d", rank));
    ...
    // inplacebuf_free = s->allreduce_cuda_config.cuda_buff[0];
    inplacebuf_free = cuda_config->cuda_buff[0];

    OPAL_OUTPUT((ompi_coll_remap_stream, "coll:remap:allreduce cudabuf1:%x cudabuf2:%x",
                 cuda_config->cuda_buff[0],
                 cuda_config->cuda_buff[1]));
    ...
    inplacebuf = inplacebuf_free;

    tmpsend = MPI_IN_PLACE == sbuf
                  ? (char *)inplacebuf
                  : sbuf;
    ...
    if (rank < (2 * extra_ranks))
    {
        ...
        else
        {
            ...
            vecAdd((float *)tmprecv, (float *)tmpsend, count);
            newrank = rank >> 1;
        }
    }
    else
    ...
    for (distance = 0x1; distance < adjsize; distance <<= 1)
    {
        ...
        vecAdd((float *)inplacebuf, (float *)rbuf, count);
    }
...
/*
 *   ompi_coll_cuda_allreduce_opt_ring
 *
 ...
 *
 */
int ompi_coll_remap_allreduce_cuda_opt_ring(const void *sbuf, void *rbuf, int count,
                                            struct ompi_datatype_t *dtype,
                                            struct ompi_op_t *op,
                                            struct ompi_communicator_t *comm,
                                            mca_coll_base_module_t *module,
                                            coll_remap_allreduce_cuda_coll_config_t *cuda_config)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:remap:allreduce_ring_cu_redcue rank %d, count %d", rank, count));
    ...
    if (count < size)
    {
        OPAL_OUTPUT((ompi_coll_base_framework.framework_output, "coll:remap:allreduce_ring_cu_reduce rank %d/%d, count %d, switching to recursive doubling", rank, size, count));
        return (ompi_coll_remap_allreduce_cuda_opt_recursivedoubling(sbuf, rbuf,
                                                                     count,
                                                                     dtype, op,
                                                                     comm, module, cuda_config));
    }
    ...
    */
    ...
    inbuf[0] = cuda_config->cuda_buff[0];
    if (NULL == inbuf[0])
    {
        ret = -1;
        line = __LINE__;
    ...
    if (size > 2)
    {
        // inbuf[1] = s->allreduce_cuda_config.cuda_buff[1];
        inbuf[1] = cuda_config->cuda_buff[1];
        if (NULL == inbuf[1])
        {
            ret = -1;
            line = __LINE__;
    ...
    */
    ...
    block_offset = ((rank < split_rank) ? ((ptrdiff_t)rank * (ptrdiff_t)early_segcount) : ((ptrdiff_t)rank * (ptrdiff_t)late_segcount + split_rank));
    ...
    for (k = 2; k < size; k++)
    {
        ...
        */
        block_offset = ((prevblock < split_rank) ? ((ptrdiff_t)prevblock * early_segcount) : ((ptrdiff_t)prevblock * late_segcount + split_rank));
        block_count = ((prevblock < split_rank) ? early_segcount : late_segcount);
        tmprecv = ((char *)rbuf) + (ptrdiff_t)block_offset * extent;
        vecAdd((float *)inbuf[inbi ^ 0x1], (float *)tmprecv, block_count);
    ...
    block_offset = ((recv_from < split_rank) ? ((ptrdiff_t)recv_from * early_segcount) : ((ptrdiff_t)recv_from * late_segcount + split_rank));
    block_count = ((recv_from < split_rank) ? early_segcount : late_segcount);
    tmprecv = ((char *)rbuf) + (ptrdiff_t)block_offset * extent;
    vecAdd((float *)inbuf[inbi], (float *)tmprecv, block_count);
    ...
    for (k = 0; k < size - 1; k++)
    {
        ...
        const int send_block_offset =
            ((send_data_from < split_rank) ? ((ptrdiff_t)send_data_from * early_segcount) : ((ptrdiff_t)send_data_from * late_segcount + split_rank));
        const int recv_block_offset =
            ((recv_data_from < split_rank) ? ((ptrdiff_t)recv_data_from * early_segcount) : ((ptrdiff_t)recv_data_from * late_segcount + split_rank));
        block_count = ((send_data_from < split_rank) ? early_segcount : late_segcount);
 ...
 */
int ompi_coll_remap_allreduce_cuda_opt_redscat_allgather(
    ...
    coll_remap_allreduce_cuda_coll_config_t *cuda_config)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:remap:allreduce_redscat_allgather_cu_reduce: rank %d/%d",
                 rank, comm_size));
    ...
    {
        OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                     "coll:remap:allreduce_redscat_allgather_cu_reudce: rank %d/%d "
                     "count %d switching to recursivedoubling",
                     rank, comm_size, count));
        return ompi_coll_remap_allreduce_cuda_opt_recursivedoubling(sbuf, rbuf, count, dtype,
                                                                    op, comm, module, cuda_config);
    }
    ...
    char *tmp_buf = NULL;
    // char *tmp_buf_raw = s->allreduce_cuda_config.cuda_buff[0];
    char *tmp_buf_raw = cuda_config->cuda_buff[0];
    if (NULL == tmp_buf_raw)
    ...
    if (rank < 2 * nprocs_rem)
    {
        ...
        if (rank % 2 != 0)
        {
            ...
            vecAdd((float *)((char *)tmp_buf + (ptrdiff_t)count_lhalf * extent),
                   (float *)((char *)rbuf + count_lhalf * extent),
                   count_rhalf);
        ...
        else
        {
            ...
            /* Reduce on the right half of the buffers (result in rbuf) */
            vecAdd((float *)tmp_buf, (float *)rbuf, count_lhalf);
    ...
    if (vrank != -1)
    {
        ...
        for (int mask = 1; mask < nprocs_pof2; mask <<= 1)
        {
            ...
            vecAdd((float *)((char *)tmp_buf + (ptrdiff_t)rindex[step] * extent),
                   (float *)((char *)rbuf + (ptrdiff_t)rindex[step] * extent),
                   rcount[step]);
