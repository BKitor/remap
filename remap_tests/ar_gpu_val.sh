...
# export OMPI_MCA_coll_remap_use_scotch=0
export OMPI_MCA_coll_remap_cc_cluster=0
export OMPI_MCA_coll_remap_turn_off_remap=0

mpicc ar_gpu_val.c -o ar_gpu_val.out -lcuda -lcudart
if [ $? -ne 0 ]; then
    exit
fi

export OMPI_MCA_coll_remap_priority=80

for i in 1 0 ; do
    for NUM_PROC in 2 4; do
        export OMPI_MCA_coll_remap_use_gpu_reduce=$i

        export OMPI_MCA_coll_remap_select_allreduce_alg=3
        mpirun -n $NUM_PROC ar_gpu_val.out
        echo "rdouble exited $? with $NUM_PROC proc" | tee >/dev/stderr

        export OMPI_MCA_coll_remap_select_allreduce_alg=4
        mpirun -n $NUM_PROC ar_gpu_val.out
        echo "ring exited $? with $NUM_PROC proc" | tee >/dev/stderr

        export OMPI_MCA_coll_remap_select_allreduce_alg=6
        mpirun -n $NUM_PROC ar_gpu_val.out
        echo "rsa exited $? with $NUM_PROC proc" | tee >/dev/stderr
    done
    export OMPI_MCA_coll_remap_priority=40
done
